...
export async function GET() {
  const { data } = await getSupabaseAdmin().auth.admin.listUsers({ page: 1, perPage: 100 })
...
export async function POST(req: NextRequest) {
  ...
  const { data: existing } = await getSupabaseAdmin().auth.admin.listUsers({ page: 1, perPage: 100 })
